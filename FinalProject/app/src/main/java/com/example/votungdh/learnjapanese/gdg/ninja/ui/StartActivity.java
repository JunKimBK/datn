...
import static com.example.votungdh.learnjapanese.gdg.ninja.ui.SCREEN_STATE.CUSTOM_GAME;
import static com.example.votungdh.learnjapanese.gdg.ninja.ui.SCREEN_STATE.START_GAME;
...
public class StartActivity extends BaseActivity implements OnClickListener {
	...
	public void onBackPressed() {
		if (dialog != null && dialog.isShowing())
			return;
		if (mScreenState == CUSTOM_GAME) {
			changeScreenState(START_GAME);
			return;
		}
	...
	public void onClick(View view) {
		...
		/*if (viewId == R.id.btn_start)
			SoundUtils.getInstance().play(SoundUtils.SOUND_NAME.START_BTN);
		else
		...
		else if(viewId == R.id.btn_custom) {
			startMakeGameAc();
		}
		else if(viewId == R.id.btn_high_score) {
			startHighScoreAc();
		}
		else if(viewId == R.id.btn_option){
			startOptionAc();
		}
