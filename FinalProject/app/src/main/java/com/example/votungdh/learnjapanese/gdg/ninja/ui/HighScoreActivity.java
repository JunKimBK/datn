...
public class HighScoreActivity extends Activity implements OnClickListener {
	...
	public void onClick(View v) {
		...
		if(viewID == R.id.btn_share_facebook){
			new TakeScreenShot(ShareUtils.SHARE_TYPE.FACEBOOK).execute();
		}
		if(viewID == R.id.btn_share_google){
			new TakeScreenShot(ShareUtils.SHARE_TYPE.GOOGLE).execute();
		}
		if(viewID == R.id.btn_choose_from_galary){
			finish();
		}
	}
}